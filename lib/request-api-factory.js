...
const RequestAPIFactory = function(apiSchema, requestConfig) {

	const request = require("request-promise").defaults(requestConfig);
	...
	function apiCall(schema, data) {
		...
		if (schema.data && schema.data.required && (schema.data.validate || typeof schema.data.validate == 'undefined')) {
			...
			let requiredOk = schema.data.required.every(item => {
				return (config.data && Object.keys(config.data).indexOf(item) >= 0);
			});

			if (!requiredOk) {
				return Promise.reject(`Missing some of the following required parameters: "${schema.data.required.join(', ')}" :: Current parameters :: ${( config.data ? Object.keys(config.data).join(', ') : "No parameters provided")}`);
			}
		}
